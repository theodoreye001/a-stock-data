...
A 股全栈数据工具包 — 11 层架构 · 44 个端点（41 主端点 + 3 官方备胎）· 15 个数据源 · 零第三方数据封装依赖
...
> **V3.5.0 技术指标层（2026-07-12）：** 新增 **§11 技术指标层（本地计算）** `technical_indicators()`——把 `stockstats`（依赖表早已列，此前无任何函数使用）封装成显式内嵌函数：**纯本地计算，不新增数据源、不发指标类网络请求**，复用 §1.1 mootdx K 线，一次算全 MACD(`macd`/`macds`/`macdh`)、KDJ(`kdjk`/`kdjd`/`kdjj`)、RSI(`rsi_6/12/24`)、布林带(`boll`/`boll_ub`/`boll_lb`)、多周期均线(`close_5/10/20/60_sma`)及 dma/atr/cci。兼容 stockstats 新版 `wrap()` / 旧版 `retype()`，并把 mootdx 的 `vol` 归一化为 `volume`。**复权警示**：mootdx bars 不复权，跨除权除息日算 MACD/KDJ/布林带会失真，跨除权日改用带前复权的腾讯/百度日 K。端点 43→44，层 10→11，数据源不变（15，纯本地计算）。
>
...
└── 技术指标  stockstats（本地计算）        MACD / KDJ / RSI / 布林带 / 多周期MA（基于 mootdx K线，不新增数据源）  ★V3.5
```
...
> **计数口径：** 下方清单共 46 行，按端点计 44 个——「东财 行业研报」与「东财 reportapi」为**同一端点**（仅 `qType` 参数不同），「同花顺北向（历史）」为本地自缓存（非独立端点），两行不重复计数。§11 技术指标为本地计算（不新增数据源），计入端点总数。
...
| 端点 | 数据 |
|------|------|
| `technical_indicators(code)` | 基于 mootdx K 线本地算 MACD(DIF/DEA/柱) / KDJ / RSI(6/12/24) / 布林带(中上下轨) / 多周期均线(MA5/10/20/60) 及 dma/atr/cci；不新增数据源、不发网络请求（stockstats 封装） |
...
| 变化 | 说明 |
|------|------|
| **新增 §11 技术指标层（本地计算）** | `technical_indicators()` 把 `stockstats`（依赖表早已列，此前无任何函数使用）封装成显式内嵌函数：复用 §1.1 mootdx K 线，一次算全 MACD / KDJ / RSI / 布林带 / 多周期均线 及 dma/atr/cci。**纯本地计算，不新增数据源、不发指标类网络请求** |
| **stockstats 新旧版兼容 + 列名归一化** | 新版 `wrap()` / 旧版 `retype()` 自动适配；mootdx 的 `vol` 归一化为 stockstats 要求的小写 `volume` |
| **复权警示（与 §1.1 一致）** | mootdx bars 不复权，跨除权除息日算 MACD/KDJ/布林带会失真；跨除权日改用带前复权的腾讯/百度日 K 后再计算 |
| **端点 43 → 44，层 10 → 11** | 数据源不变（15，纯本地计算）；When to Activate / 关键词 / 端点路由总表同步补技术指标 |

> 更早版本亮点见下方与 [CHANGELOG.md](./CHANGELOG.md)。
...
Full-stack data toolkit for China A-Share market — 11-layer architecture · 44 endpoints (41 primary + 3 official backups) · 15 data sources · zero third-party data wrapper dependencies
...
> **V3.5.0 — Technical Indicators layer (2026-07-12):** New **§11 Technical Indicators layer (local compute)** `technical_indicators()` — wraps `stockstats` (long listed in the dependency table but never used by any function until now) into an explicit embedded function: **pure local computation, no new data source and no indicator-related network requests**, reusing §1.1 mootdx K-lines to compute MACD (`macd`/`macds`/`macdh`), KDJ (`kdjk`/`kdjd`/`kdjj`), RSI (`rsi_6/12/24`), Bollinger Bands (`boll`/`boll_ub`/`boll_lb`), multi-period MAs (`close_5/10/20/60_sma`) and dma/atr/cci in one call. Compatible with stockstats new `wrap()` / old `retype()`, and renames mootdx's `vol` to the required `volume`. **Adjustment caveat:** mootdx bars are unadjusted, so MACD/KDJ/Bollinger across ex-dividend dates get distorted — use forward-adjusted Tencent/Baidu daily K across those dates. Endpoints 43→44, layers 10→11, data sources unchanged (15, pure local compute).
>
...
```
China A-Share Full-Stack Data · 11-Layer Architecture · V3.5.0
...
└── Technicals     stockstats (local compute)         MACD / KDJ / RSI / Bollinger / multi-period MA (from mootdx K-lines, no new source)  ★V3.5
```
...
> **Counting convention:** the tables below have 46 rows but count as 44 endpoints — "Eastmoney Industry Reports" shares **the same endpoint** as "Eastmoney reportapi" (only the `qType` parameter differs), and "THS Northbound (historical)" is a local self-built cache (not a separate endpoint); neither is double-counted. §11 Technical Indicators is local compute (no new data source) but counts toward the endpoint total.
...
### Technical Indicators (V3.5 new · local compute)

| Endpoint | Data |
|----------|------|
| `technical_indicators(code)` | MACD (DIF/DEA/hist) / KDJ / RSI(6/12/24) / Bollinger (mid·upper·lower) / multi-period MA (5/10/20/60) & dma/atr/cci, computed locally from mootdx K-lines; no new data source, no network requests (stockstats wrapper) |
...
| Change | Description |
|--------|-------------|
| **New §11 Technical Indicators layer (local compute)** | `technical_indicators()` wraps `stockstats` (long listed in deps, never used until now) into an explicit embedded function: reuses §1.1 mootdx K-lines to compute MACD / KDJ / RSI / Bollinger / multi-period MA & dma/atr/cci in one call. **Pure local computation — no new data source, no indicator-related network requests** |
| **stockstats new/old compatibility + column normalization** | Auto-adapts new `wrap()` / old `retype()`; renames mootdx's `vol` to the lowercase `volume` stockstats requires |
| **Adjustment caveat (consistent with §1.1)** | mootdx bars are unadjusted, so MACD/KDJ/Bollinger get distorted across ex-dividend dates; use forward-adjusted Tencent/Baidu daily K across those dates |
| **Endpoints 43 → 44, layers 10 → 11** | Data sources unchanged (15, pure local compute); When-to-Activate / keywords / endpoint routing table all updated with technical indicators |

> Earlier version highlights: see below and [CHANGELOG.md](./CHANGELOG.md).
